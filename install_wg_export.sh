...
$user_groups = is_array($_SESSION['Groups']) ? $_SESSION['Groups'] : [];
if ($_SESSION['Username'] !== 'admin' && !in_array('admins', $user_groups)) {
...
if (session_status() == PHP_SESSION_NONE) { session_start(); }
$user_groups = is_array($_SESSION['Groups']) ? $_SESSION['Groups'] : [];
$is_admin = ($_SESSION['Username'] === 'admin' || in_array('admins', $user_groups));
